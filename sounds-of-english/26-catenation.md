...
>   * 省音（ *elision*，[2.2.5.1](17-td.html#_2-2-5-1-省音)）
>   * 叠音（*geminates*，[2.2.5.2](17-td.html#_2-2-5-2-叠音)）
>   * 同化（*assimilation*，[2.2.5.3](17-td.html#_2-2-5-3-同化)）；
>  * 弹舌音 （*alveolar flapping*，[2.2.5.4](17-td.html#_2-2-5-4-弹舌音)）
> * `h` 和 `t` 可能会被击穿（*dropping*, [2.2.8](20-h)，[2.2.5.5](17-td.html#_2-2-5-5-击穿)）；
> * `ŋ` 可能会读成 `n`（[2.2.3](15-mn)）；
> * `j` 和 `w` 可能会不知不觉地被插入，形成加音（*intrusion*, [2.2.13](25-jw)）
> * 相同或相似的辅音叠音（*geminates*，[2.2.5.2](17-td.html#_2-2-5-2-叠音)）
...
> * *with the*，`wɪθ ðə`，`θ· ð` 两个相近的辅音叠在一起，前一个只剩下了停顿，`wɪ·ðə`（[2.2.5.2](17-td.html#_2-2-5-2-叠音)）；
> * *got to*，`gɒt tʊ`，两个 `t` 叠在一起，前一个只剩下了停顿（[2.2.5.2](17-td.html#_2-2-5-2-叠音)），后面的 `to` 被弱读（[3.5](#3.5-轻重)）成了 `tə`，甚至听起来像是单独的 `t`： `gɒ·t(ə)`；
